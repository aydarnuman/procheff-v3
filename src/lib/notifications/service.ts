  ...
  async updateChannel(channelId: number, updates: Partial<NotificationChannel>): Promise<void> {
    try {
      ...
      if (updates.destination !== undefined) {
        ...
        updateFields.push(`verification_expires = $${paramIndex}`);
      ...
      await db.execute(
        `UPDATE notification_channels SET ${updateFields.join(", ")} WHERE id = $${paramIndex}`,
        values
      );
  ...
  async queueNotification(
    ...
    try {
      ...
      const channel = await db.queryOne(`
        ...
        WHERE user_id = $1 AND type = $2 AND verified = 1
        LIMIT 1
