...
export class TwoFactorAuthService {
  ...
  async generateSetup(userId: string, userEmail: string): Promise<TwoFactorSetup> {
    ...
    if (existingUser) {
      ...
      await db.execute(
        `UPDATE user_2fa
         SET secret = $1, backup_codes = $2, enabled = $3, created_at = CURRENT_TIMESTAMP
         WHERE user_id = $4`,
        [secret.base32, JSON.stringify(backupCodes), false, userId]
      );
    } else {
      // Insert new record
      await db.execute(
        ...
        [userId, secret.base32, JSON.stringify(backupCodes), false]
      );
    }
  ...
  async verifyAndEnable(userId: string, token: string): Promise<boolean> {
    ...
    if (verified) {
      // Enable 2FA
      await db.execute(
        "UPDATE user_2fa SET enabled = $1, last_used = CURRENT_TIMESTAMP WHERE user_id = $2",
        [true, userId]
      );
  ...
  async verifyToken(userId: string, token: string): Promise<boolean> {
    const db = await getDatabase();
    const user2fa = await db.queryOne(
      "SELECT * FROM user_2fa WHERE user_id = $1 AND enabled = $2",
      [userId, true]
    ) as User2FA | undefined;
  ...
  async disable(userId: string): Promise<void> {
    const db = await getDatabase();
    await db.execute("UPDATE user_2fa SET enabled = $1 WHERE user_id = $2", [false, userId]);
